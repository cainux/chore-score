...
- **A parent-only admin page** with the same two-week grid, but every square is tappable to toggle a sticker on or off, including days in the past. It also edits each child's display name and task bullet list. Every control carries the date it was rendered for, so a tap that lands after midnight records the day the parent was looking at.
- **Editable display names**, so a parent can decorate a child's name with emoji for a birthday or a holiday. The roster itself stays fixed. Emoji render from a bundled monochrome font, because colour glyphs do not survive a 2-bit panel and a system font is a dependency the display cannot afford.
...
- **A weekly trophy** awarded when a child earns a sticker on all 7 days of a week, drawn in the same solid black as the stickers themselves. A week that is not complete shows nothing: the panel proved that a hollow trophy standing for "still winnable" did not read as a trophy across a kitchen, so the mark now means one thing. The app records that the trophy was earned and displays it; it does not know or track what the actual prize is.
