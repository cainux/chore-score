...
- [x] 1.4a Install `@cloudflare/vitest-pool-workers` — it is not currently a dependency, though `pnpm-workspace.yaml` already allows the `workerd` build for it. The release line peering `vitest ^4.1` pins the `wrangler` and `miniflare` versions already in the lockfile, so it should resolve without moving anything else; check that it does
- [x] 1.5 Add the `workers` vitest project per design.md D9, standalone rather than extending `vite.config.ts`, matching `src/**/*.workers.spec.ts` — and add that same pattern to the `server` project's `exclude` so the files do not also run in Node
...
- [x] 4.2 Implement day-square state resolution: earned / missed / not-yet, treating today as missed rather than not-yet when unmarked
- [x] 4.3 Implement week winnability per design.md D13: lost when any date **strictly before** today is unmarked, still winnable otherwise, complete taking precedence over both — deliberately not the same rule as 4.2, and not built on it — **reversed in 8.4c**: the panel retired the hollow trophy, so there are no partial states left to report and the rule was removed. What remains is `weekComplete`, which asks nothing about today
- [x] 4.4 Implement task-text-to-bullets conversion per design.md D12: split on line breaks, trim, drop empties, strip a leading `-`/`*`/`•`; plain text only, no Markdown, stored text left untouched
- [x] 4.5 Test that a current week with a perfect record so far is not complete, and that clearing one day of a complete week revokes the trophy — in the `workers` project, since completion is a D1 query. Test square-state resolution, winnability, and bullet conversion in the `server` project, since they are pure
- [x] 4.6 Test the winnability flicker case specifically: with every earlier day marked and today unmarked, the week is still winnable and does not flip to lost — the failure mode if 4.3 is built on 4.2 — **retired with 4.3 in 8.4c**. The flicker was a property of the three-state trophy; with won-or-nothing there is no state for it to flicker between. The replacement test asserts the stronger property that made it moot: completion asks nothing about today
- [x] 4.7 Put every value that step 8.4 will retune into one module with provisional settings: the name length limit in graphemes (7.7, 7.13), the bullet limit (6.8), and the display typeface (6.1). Sections 6 and 7 cannot be written without a value for each, but none can be decided before the panel is in front of you — so the point is that 8.4 edits one file rather than hunting for scattered numbers. The typeface is not a domain rule and sits here only to keep the retune in one place
...
- [x] 5.6 Test that an admin session does not open `/display`, and that the display header does not open `/admin` — **revised**: the first half was relaxed in 5.7. The display-key-to-admin direction is unchanged and still asserted
- [x] 5.7 Let an admin session open `/display` too, so the wall chart can be previewed from a phone, and link to it from the admin page. Keep the reverse shut: the display key stays read-only and must never open `/admin` (design.md D4, revised)
...
- [x] 6.5 Render the trophy in all three states per design.md D13 — solid `#555` won, outline `#555` winnable, outline `#AAA` lost — on both week rows, so the slot is never empty — **revised in 8.4c**: one state, solid `#000`, drawn only on a complete week; the slot keeps its width but is otherwise empty
...
- [x] 8.2 Point the TRMNL Screenshot plugin at the preview URL with the custom header; confirm it captures successfully
- [x] 8.3 Check the capture on the physical panel: are the squares legible at kitchen distance, does the font survive 2-bit conversion, are the three square states distinguishable, is a solid trophy unmistakable from a hollow one across the room, does the faint trophy stay clear of the `#AAA` grid lines around it, and does a monochrome emoji in a name read as a picture at name size — **answered across three rounds**. The first plugin capture settled the task type and the stamp (8.4a). The panel then settled the trophies, and the answer was that hollow ones do not read at all — which retired the two questions comparing them (8.4c). The remaining questions, the three square states at kitchen distance and emoji at name size, checked out on the panel with nothing left to change
- [x] 8.4 Adjust type sizes, weights, and grey levels based on what the panel actually shows; decide the typeface open question here, whether the won trophy needs to be black rather than `#555`, and settle the name length limit against real emoji names — the provisional values are collected in the module from task 4.7 — **settled over 8.4a–8.4c**. Type sizes, grey levels and the trophy colour all moved and are recorded there. The two remaining open questions closed without a change: the bundled typeface survives 2-bit conversion at every size the chart uses, and 16 graphemes holds a real emoji name inside the 80px gutter. Those values are no longer provisional
- [x] 8.4a First round of that retune, from the first capture: task bullets 16px → 20px in a 230px band of 8 whole 25px lines; bullets wrap instead of being cut off mid-word; the render stamp 12px `#AAA` → 14px `#555`, because `#AAA` dithered into a smudge; bullet limits down to 6 comfortable / 8 maximum to match (design.md D6, D10)
- [x] 8.4b Read inline emphasis in task bullets — `*italic*`, `**bold**`, `***both***` and nothing else — with unmatched delimiters rendered as typed, and bundle real italic faces rather than letting the renderer synthesise a slant (design.md D12, revised)
- [x] 8.4c Second round of the retune, from the panel: drop the bullet markers and their 22px indent so a task is a plain line at the full 376px column width; take the won trophy from `#555` to `#000`, the same ink as the stars, because at `#555` the reward read as fainter than the days that earned it; and show a trophy **only** on a complete week, since a hollow one did not read as a trophy at distance. The last of those retires the winnable/lost rule and every question the trophy asked about today (design.md D12, D13, both revised; tasks 4.3, 4.6, 6.5)
- [x] 8.5 Promote to the production URL and set the TRMNL refresh interval — settle it together with the render stamp, since an overnight sleep makes a healthy chart show last night's time every morning (design.md D10) — production is `https://chores.oha.me/display`, which is what the TRMNL plugin is pointed at, and the refresh interval is **15 minutes**. The stamp is unchanged: the rule that makes that safe is that the panel must wake at least once after midnight before anyone first looks, which keeps the date correct at every hour the chart is actually read. The custom domain is now declared in `wrangler.jsonc` rather than only in the dashboard, because Wrangler overrides dashboard routes on deploy from whatever the config says — and it said nothing
- [x] 8.6 Enter real task lists for both children and confirm a full round trip: toggle a square on the phone, see it appear on the wall — both trophy slots will be empty on the first render, since last week has no data, and that is now simply correct rather than something to warn about (8.4c)
