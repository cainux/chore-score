...
- [ ] 6.4 Deploy, then confirm the panel's next capture is unchanged from the one before it (the extraction's real acceptance test). **Deployed** (version `891ab995`, `chores.oha.me` still attached); production gates verified closed — `/display` 401s without the key and `/admin/preview` redirects to the login. **The capture itself is still to be seen**, and until the panel has refreshed once this change is not finished: a broken capture leaves the last good image on the wall and announces nothing.
