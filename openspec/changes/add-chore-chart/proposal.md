...
- **A read-only display page** sized for the TRMNL OG (800×480, 2-bit greyscale), fetched and screenshotted by TRMNL's Screenshot plugin. It shows each child's task bullets plus a two-week sticker grid (last week and this week, Monday start).
- **A parent-only admin page** with the same two-week grid, but every square is tappable to toggle a sticker on or off, including days in the past. It also edits each child's task bullet list.
...
- **A weekly trophy** awarded when a child earns a sticker on all 7 days of a week. The app records that the trophy was earned and displays it; it does not know or track what the actual prize is.
