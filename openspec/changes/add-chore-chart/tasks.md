...
- [x] 4.3 Implement week winnability per design.md D13: lost when any date **strictly before** today is unmarked, still winnable otherwise, complete taking precedence over both — deliberately not the same rule as 4.2, and not built on it
...
- [x] 4.6 Test the winnability flicker case specifically: with every earlier day marked and today unmarked, the week is still winnable and does not flip to lost — the failure mode if 4.3 is built on 4.2
...
- [ ] 8.3 Check the capture on the physical panel: are the squares legible at kitchen distance, does the font survive 2-bit conversion, are the three square states distinguishable, is a solid trophy unmistakable from a hollow one across the room, does the faint trophy stay clear of the `#AAA` grid lines around it, and does a monochrome emoji in a name read as a picture at name size — **partly answered** from the first plugin capture, which settled the task type and the stamp (8.4a). Still open on the panel itself: the trophies, the square states at distance, and emoji at name size
