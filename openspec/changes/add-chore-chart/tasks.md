...
- [ ] 8.3 Check the capture on the physical panel: are the squares legible at kitchen distance, does the font survive 2-bit conversion, are the three square states distinguishable, is a solid trophy unmistakable from a hollow one across the room, does the faint trophy stay clear of the `#AAA` grid lines around it, and does a monochrome emoji in a name read as a picture at name size — **mostly answered**. The first plugin capture settled the task type and the stamp (8.4a); the panel then settled the trophies, and the answer was that hollow ones do not read at all (8.4c), which retires the two questions about them. Still open on the panel itself: the three square states at distance, and emoji at name size
- [ ] 8.4 Adjust type sizes, weights, and grey levels based on what the panel actually shows; decide the typeface open question here, whether the won trophy needs to be black rather than `#555`, and settle the name length limit against real emoji names — the provisional values are collected in the module from task 4.7
...
- [ ] 8.6 Enter real task lists for both children and confirm a full round trip: toggle a square on the phone, see it appear on the wall — both trophy slots will be empty on the first render, since last week has no data, and that is now simply correct rather than something to warn about (8.4c)
